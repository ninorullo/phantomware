...
### malconv 
Contains a Phantomware-compatible version of MalConv2, a malware detector proposed in the AAAI 2021 paper
[Classifying Sequences of Extreme Length with Constant Memory Applied to Malware Detection](https://arxiv.org/abs/2012.09390).

### phantomware

Contains the Phantomware framework. 

### models

Stores the pre-trained MalConv model generated during the initial training phase, and the fine-tuned MalConv 
model produced after completing the fine-tuning process.

### data

Contains both the training and fine-tuning datasets. The `real_data` subdirectory holds real
malware and goodware samples used to train the MalConv model; the `synthetic_data` subdirectory contains 
synthetic malware representations generated by Phantomware for subsequent fine-tuning.
